/*jslint devel: true, indent: 2, node: true, browser: true, white: true*/
...
(function (global) {
  ...
  var gui   = require("nw.gui")
    ...
    , updateParcel
    , cm
    , saveFile, filePath, fileDirty, setFileDirty, newFile, openFileDialog
    , openFile, toggleFullscreen, toggleAudio, closeWindow, saveAndClose
    , findmenu, filemenu, viewmenu, thememenu, editmenu, openmenu, openrecents
    , recentFiles, updateRecentFiles, clearRecentFiles, hasRecentFiles
    , removeRecentFile, completeInit, setPageTitle, updateTitleDirt
    , openSettings, displayWordCount, getWordCount, loadDefaultTheme
    , unloadDefaultTheme, loadTheme, setDefaultTheme, getDefaultTheme
    , submenuLoadTheme, updateCounterDirt
    , toggleSuperfluous, $scrollbar, $counter
    , playClicks;
  ...
  toggleSuperfluous = function (hide, override) {
    // "override" is for special case when app leaves fullscreen and needs to
    // unhide all superfluous.
      ...
      if (hide) {
        if (counterCss === "block" || scrollCss === 1) {
          $scrollbar.fadeOut(duration, function () {
            // gotta set the scrollbar opacity to 0 since cm automatically sets
            // vscrollbar to block on key event, making the fadeout useless.
        ...
        if (counterCss === "none" || scrollCss === 0) {
          ...
          $scrollbar.fadeIn(duration, function () {
            // redundant but at least it fades in nicely for the user.
            $scrollbar.css("opacity", 1);
          });
          if (win.isFullscreen) {
  ...
  updateRecentFiles = function (path) {
    ...
    // place element at the top of the array by using unshift instead of push.
  ...
  openSettings = function () {
    var customizer, themes, close, save, hider, updateTheme
      , updateElement, styleDiv
      , bgimg, bgimgy, bgimgx, bgimgcover, bgcolor, textfont, textsize
      , textsizeunit, textweight, textstyle, textcolor, texthighlight
      , scrollcolor, scrolltrackcolor, saveTheme, allowaudio, allowclicks
      , audioselect, clickselect;
    ...
    $("#wr-bg-color").spectrum({
      ...
      showInput: true,
      move: function (color) {
        updateElement("body", theme.body, "background-color", color.toPercentageRgbString());
        updateTheme();
      },
      hide: function (color) {
        updateElement("body", theme.body, "background-color", color.toPercentageRgbString());
    ...
    bgimg.onchange = function () {
      var img = bgimg.value;
      if (img !== "") {
        updateElement("body", theme.body, "background-image", "url('"+img+"')");
        theme.bgImg = img;
      } else {
    ...
    bgimgy.onchange = function () {
      if (!bgimgy.checked) {
        if (!bgimgx.checked) {
          updateElement("body", theme.body, "background-repeat", "no-repeat");
        } else {
          updateElement("body", theme.body, "background-repeat", "repeat-x");
        }
      } else {
        if (bgimgx.checked) {
          updateElement("body", theme.body, "background-repeat");
        } else {
          updateElement("body", theme.body, "background-repeat", "repeat-y");
        }
      }
      updateTheme();
    };
    ...
    $("#wr-text-color").spectrum({
      ...
      showInput: true,
      move: function (color) {
        updateElement("text", theme.cm, "color", color.toPercentageRgbString());
        updateTheme();
      },
      hide: function (color) {
        updateElement("text", theme.cm, "color", color.toPercentageRgbString());
    ...
    textfont.onchange = function () {
      updateElement("text", theme.cm, "font-family", "'" + textfont.value + "'");
      updateTheme();
    };
    ...
    textsize.onchange = function () {
      updateElement("text", theme.cm, "font-size", textsize.value + textsizeunit.value);
      updateTheme();
    };
    ...
    saveTheme = function (themeName, themesPath) {
      ...
      compileCss = function (fullPath, callback) {
        ...
        if (theme.body.length !== 0) {
          ...
          for (i = 0; i < theme.body.length; i += 1) {
            if (theme.body[i].name === "background-image") {
              ...
              bod += theme.body[i].name + ":url('" + bgThemePath + "');";
            } else {
              bod += theme.body[i].name + ":" + theme.body[i].value + ";";
            }
          }
          bod += "} ";
        }
        ...
        for (i = 0; i < theme.other.length; i += 1) {
          ...
          oth += theme.other[i].name + ":" + theme.other[i].value + ";";
          oth += "} ";
        }
      ...
      writeCss = function (fullPath, themeName, css, callback) {
        var cssName = themeName + ".css"
          ...
          , fileName, imgThemePath, bgThemePath, getImgStr, putImgStr;
        if (theme.bgImg) {
          fileName = theme.bgImg.split("/").pop();
  ...
  updateTitleDirt = function (isDirty) {
    ...
    if (oldTitle.indexOf(dirt) >= 0) {
      if (oldTitle.slice(0, dirt.length) === dirt) {
        // dirt char found at start of doc title. assume this is indication of 
        // dirt and not just user's own file name.
        oldDirt = true;
  ...
  win.on("close", function () {
    // if file has been dirtied & codemirror history is not already at oldest undo
    if (fileDirty) {
